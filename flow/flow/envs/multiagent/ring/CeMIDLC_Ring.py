...
class TD3CeMILCAccelEnv(MultiEnv):
    ...
    def Coef(self):
        ...
        if self.time_counter == 0:
            coef_log = []
            # coef_log2 = []
            rl_des = 1.5 #random.sample([i for i in np.arange(0,2,0.1)],1)[-1]
            uns4IDM_p = 0.1 #random.sample([i for i in np.arange(0, 2, 0.1)], 1)[-1]
            mlp = .5 #random.sample([i for i in np.arange(0,2,0.1)],1)[-1]
            
            coef_log.append([rl_des, uns4IDM_p, mlp])
            # coef_log1.append([rl_des, uns4IDM_p, mlp])
    ...
    def compute_reward(self, rl_actions, **kwargs):
            ...
            if rl_des:
                if self.k.vehicle.get_speed(rl) > 0.:
                    ...
                    if any(vel < -100) or kwargs['fail']:
                        return 0.
                    if self.env_params.additional_params['target_velocity']:
        ...
        # f = open("/home/bmil/LEE/flowdata/Reward{}.csv".format(len(reward.keys())), 'a')
        # f.write('{}\n'.format(reward))
        # f.close()
    ...
    def _apply_rl_actions(self, actions):
        ...
        acceleration = [i[::2] for i in actions.values()]
        direction = [step_direction(i[1::2]) for i in actions.values()]
...
class TD3CeMILCAccelPOEnv(TD3CeMILCAccelEnv):
