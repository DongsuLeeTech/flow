...
class QMIXForwardLaneChange3RingEnv(MultiEnv):
    ...
    def Coef(self):
        ...
        if self.time_counter == 0:
            coef_log = []
            des = list(np.random.choice([i for i in np.arange(1.0, 2.1, 0.1)], len(rls), replace=True))
            uns = list(np.random.choice([i for i in np.arange(0.0, 1.0, 0.1)], len(rls), replace=True))
            mlp = list(np.random.choice([i for i in np.arange(0.2, 1.0, 0.1)], len(rls), replace=True))
...
class QMIXForwardLaneChange3RingPOEnv(QMIXForwardLaneChange3RingEnv):
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        super().__init__(env_params, sim_params, network, simulator)
