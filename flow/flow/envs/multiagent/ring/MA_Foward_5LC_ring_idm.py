...
class MARingIDMEnv(MultiEnv):
    ...
    def compute_reward(self, actions, **kwargs):
        ...
        # print(self.prev_lc_action, self.prev_my_lane, self.my_lane)
        reward_dict = {}
        ...
        for i, rl in enumerate(rls):
            if kwargs['fail'] or self.crash:
                ...
                #bmil edit 02/06: 차선 변경 사고 penalty 추가
                # if self.prev_lc_action is not None:
                #     if self.prev_my_lane[i]+self.prev_lc_action[i] != my_lane:
                #         print(self.prev_lc_action[i], self.prev_my_lane[i], my_lane)
                #         self.crash=True
                #         rwd = default_reward + accident_p
                ...
                if len(same_lane_ids) == 0:
                    ...
                    # print(np.any(self.num_lanes != self.max_lanes))
                    ...
                    if (self.my_edge[i] == self.shring_junction) and np.any(self.num_lanes != self.max_lanes):
                        ...
                        rl_pos = np.array(self.k.vehicle.get_position(rl))
                        sj_pos = np.array(self.k.vehicle.get_position(sj_ids))
                        ...
                        if len(sj_leader_pos)>0:
                            ...
                            leader_pos = self.k.vehicle.get_position(leader_id)[0]
                            # print(leader_pos)
                ...
                if self.crash==False:
                    if rl_des:
                        if self.k.vehicle.get_speed(rl) >= 0.:
                            ...
                            # print(self.k.vehicle.get_speed(rl))
                            # time.sleep(10)
                            return 0.
                    ...
                    if (mlp) and (self.prev_my_lane[i] is not None):
                        ...
                        #print(self.prev_leader_headway)
                    if rl_action_p:
                    ...
                    # print(rwds['meaningless_penalty'])
...
class MADRingLCPOIDMEnv(MARingIDMEnv):
    """POMDP version of LaneChangeAccelEnv.
        Required from env_params:
        * max_accel: maximum acceleration for autonomous vehicles, in m/s^2
        * max_decel: maximum deceleration for autonomous vehicles, in m/s^2
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        self.observable_lanes = 3 #실제 visibile lane 수
        self.vehicle_length = 4.45
    ...
    def get_idm_state(self):
        """See class definition."""
        obs = {}
        length = self.k.network.length()
        # rl 차량 id
        rl_ids = self.k.vehicle.get_ids()[-1:]
