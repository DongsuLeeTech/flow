...
class TD3MADLC_DESAccelEnv(MultiEnv):
    ...
    def Coef(self):
        ...
        if self.time_counter == 0:
            coef_log = []
            rl_des = random.sample([i for i in np.arange(1.5, 2.1, 0.1)], 1)[-1]
            uns4IDM_p = random.sample([i for i in np.arange(0.1, 0.6, 0.1)], 1)[-1]
            mlp = random.sample([i for i in np.arange(0.1, 0.6, 0.1)], 1)[-1]
            velo = random.sample([i for i in np.arange(3.5, 4.09, 0.5)], 1)[-1]
            
            coef_log.append([rl_des, uns4IDM_p, mlp, velo])
        ...
        return rl_des, uns4IDM_p, mlp, velo
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        # Training
        rl_des, uns4IDM_p, mlp, tv = self.Coef()
...
class TD3MADLC_DESAccelPOEnv(TD3MADLC_DESAccelEnv):
    ...
    def observation_space(self):
        ...
        return Box(
            low=-1,
            high=1,
            shape=(18*self.initial_vehicles.num_rl_vehicles, ),
        ...
        for rl_id in self.k.vehicle.get_rl_ids():
            ...
            for i in range(0, max_lanes):
                ...
                if self.k.vehicle.get_lane(rl_id) == i:
                    ...
                    observation = np.array(speeds + positions + lanes + coef + target_velo)
