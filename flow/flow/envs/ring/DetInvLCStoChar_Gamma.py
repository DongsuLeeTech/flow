...
class TD3LCIStoGammaAccelEnv(AccelEnv):
    ...
    def Coef(self):
        ...
        if self.time_counter == 0:
            ...
            rl_des_alpha = random.sample([i for i in np.arange(0.3, 0.8, 0.1)], 1)[-1]
            uns4IDM_p_alpha = random.sample([i for i in np.arange(0.3, 0.8, 0.1)], 1)[-1]
            mlp_alpha = random.sample([i for i in np.arange(0.3, 0.8, 0.1)], 1)[-1]


            coef_log.append([rl_des_alpha, uns4IDM_p_alpha, mlp_alpha])

            C1 = self.poisson_dist(rl_des_alpha, 0, 20)
            C2 = self.poisson_dist(uns4IDM_p_alpha, 0, 20)
            C3 = self.poisson_dist(mlp_alpha, 0, 20)
            c_log.append([C1, C2, C3])

        else:
            rl_des_alpha = coef_log[0][0]
            uns4IDM_p_alpha = coef_log[0][1]
            mlp_alpha = coef_log[0][2]

            C1 = c_log[0][0]
            C2 = c_log[0][1]
            C3 = c_log[0][2]

        return C1, C2, C3, rl_des_alpha, uns4IDM_p_alpha, mlp_alpha

    def execution_coef(self, C1_alpha, C2_alpha, C3_alpha):
        global coef_log
        global c_log
        np.set_printoptions(precision=4)
        ...
        if self.time_counter == 0:
            ...
            rl_des_alpha = C1_alpha
            uns4IDM_p_alpha = C2_alpha
            mlp_alpha = C3_alpha
    ...
    def gamma_dist(self, alpha, a, b):
        beta = 0.1
        A = Symbol('A')
        pdf = A * (A ** (alpha - 1) * np.exp(1) ** (-A * beta) * (beta ** alpha) / gamma(alpha))
        c = Integral(pdf, (A, a, b)).doit().evalf()
        return 0.1 * float(c)
...
class TD3LCIStoGammmaAccelPOEnv(TD3LCIStoGammaAccelEnv):
