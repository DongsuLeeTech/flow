...
class BLCAccelEnv(AccelEnv):
    ...
    * sort_vehicles: specifies whether vehicles are to be sorted by position
    ...
    def _apply_rl_actions(self, actions):
        ...
        """
        Method that it choose direction
        using softmax 
        """
        direction = actions[1:]

        d_list = new_softmax(direction)
...
class BLCAccelPOEnv(BLCAccelEnv):
    ...
    def get_state(self):
        ...
        lane_followers_speed = self.k.vehicle.get_lane_followers_speed(rl) + np.random.normal(0,5)
        lane_leaders_speed = self.k.vehicle.get_lane_leaders_speed(rl) + np.random.normal(0,5)
        ...
        lane_followers_pos = [self.k.vehicle.get_x_by_id(follower) + np.random.normal(0, 5) for follower in lane_followers]
        lane_leaders_pos = [self.k.vehicle.get_x_by_id(leader) + np.random.normal(0, 5) for leader in lane_leaders]
        ...
        rel_speeds = [(speed - self.k.vehicle.get_speed(rl_id)) / max_speed
                        for speed in lane_followers_speed + lane_leaders_speed]
        rl_speeds = [speed / max_speed
                        for speed in rl_speed]
        ...
        observation = np.array(rel_speeds + rl_speeds + positions + lanes)
        return observation
