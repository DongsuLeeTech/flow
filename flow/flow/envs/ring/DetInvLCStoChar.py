...
class TD3LCIStoCharAccelEnv(AccelEnv):
    ...
    def Coef(self):
        ...
        if self.time_counter == 0:
            ...
            rl_des_mu = random.sample([i for i in np.arange(1.0, 2.0, 0.1)], 1)[-1]
            uns4IDM_p_mu = random.sample([i for i in np.arange(0, 1.0, 0.1)], 1)[-1]
            mlp_mu = random.sample([i for i in np.arange(0, 1.0, 0.1)], 1)[-1]

            std = random.choice([0.5, 1])
            rl_des_std = std
            uns4IDM_p_std = std
            mlp_std = std
            coef_log.append([rl_des_mu, uns4IDM_p_mu, mlp_mu, rl_des_std, uns4IDM_p_std, mlp_std])
            if std != 0:
                C1 = self.truncated_dist(rl_des_mu, rl_des_std, 1, 2)
                C2 = self.truncated_dist(uns4IDM_p_mu, uns4IDM_p_std, 0, 1)
                C3 = self.truncated_dist(mlp_mu, mlp_std, 0, 1)
            ...
            rl_des_mu = coef_log[0][0]
            uns4IDM_p_mu = coef_log[0][1]
            mlp_mu = coef_log[0][2]
            rl_des_std = coef_log[0][3]
            uns4IDM_p_std = coef_log[0][4]
            mlp_std = coef_log[0][5]
            C1 = c_log[0][0]
            C2 = c_log[0][1]
            C3 = c_log[0][2]

        return C1, C2, C3, rl_des_mu, uns4IDM_p_mu, mlp_mu, rl_des_std, uns4IDM_p_std, mlp_std

    def execution_coef(self, C1_mu, C2_mu, C3_mu, std):
        global coef_log
        global c_log
        np.set_printoptions(precision=4)
        if self.time_counter == 0:
    ...
    def truncated_dist(self, mu, std, a, b):
        C = Symbol('C')
        A = Symbol('A')
        mu = round(mu, 2)
        if mu == 1.3:
        ...
        a = a
        b = b
        phi = (1 / float(sqrt(2 * pi))) * exp((-1 / 2) * ((A - mu) / sigma) ** 2)
        aPhi = (1 + erf((a - mu) / (sigma * sqrt(pi)))) / 2
        bPhi = (1 + erf((b - mu) / (sigma * sqrt(pi)))) / 2
        f = (1 / sigma) * (phi / (bPhi - aPhi))
        eps = 10**(-1)
        g = C * Integral(f, (A, C - eps, C)).doit()
        c = Integral(g, (C, a, b)).doit().evalf()
        # print(c, mu, sigma)
        return 10 * float(c)
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        # Training
        C1, C2, C3, _, _, _, _, _, _ = self.Coef()
        ...
        # std = self.initial_config.reward_params.get('char_std', 0)
        #
        # C1, C2, C3, _, _, _, _, _, _ = self.execution_coef(rl_des, uns4IDM_p, mlp, std)
...
class TD3LCIStoCharAccelPOEnv(TD3LCIStoCharAccelEnv):
    ...
    def get_state(self):
        ...
        # _, _, _, rl_des_mu, uns4IDM_p_mu, mlp_mu, rl_des_std, uns4IDM_p_std, mlp_std\
        #     = self.Coef()
        ...
        #
        # _, _, _, rl_des_mu, uns4IDM_p_mu, mlp_mu, rl_des_std, uns4IDM_p_std, mlp_std\
        #     = self.execution_coef(rl_des, uns4IDM_p, mlp, std)
        ...
        # Coef scaling
        char_mu = [i for i in [rl_des_mu - 1, uns4IDM_p_mu, mlp_mu]]
        char_std = [i for i in [rl_des_std, uns4IDM_p_std, mlp_std]]
        observation = np.array(speeds + positions + lanes + char_mu + char_std)
